...
function openAccountEditor(id){
  ...
  document.getElementById("acctmodalbody").innerHTML=`
    <table class="kv">
      ...
      <tr><td class="k">Weekly KPI sheet URL <span class="cc">(the KPI pack)</span></td><td class="v"><input class="ed" id="ac_weekly_url" value="${esc(a.weekly_sheet_url||'')}" placeholder="https://docs.google.com/spreadsheets/d/…"><input class="ed" id="ac_weekly_tab" value="${esc(a.weekly_sheet_tab||'')}" placeholder="Tab name — defaults to &quot;Weekly KPIs&quot;" style="margin-top:4px"><div class="cc" style="font-size:11px;margin-top:3px"><b>Deliberately separate from the output sheet above.</b> That one holds generated listings; a KPI grid written over it would destroy them. Sync writes a whole tab of its own, so a hand-made tab in the same workbook is untouched and can reference it by formula. <b>Share the sheet (Editor) with the service account</b> shown below.</div></td></tr>
...
async function saveAccount(){
  ...
  const body={
    ...
    weekly_sheet_url:((document.getElementById("ac_weekly_url")||{}).value||"").trim(),
    weekly_sheet_tab:((document.getElementById("ac_weekly_tab")||{}).value||"").trim(),
...
function navTo(sec){
  ...
  if(sec==="ppclive"){ if(typeof ppclLoad==="function" && !PPCL.data) ppclLoad(); }

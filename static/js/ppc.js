...
function ppcAppendChat(who, text){
  ...
  const who_style = (who==="you") ? "color:var(--accent2);font-weight:600" : "color:var(--ok);font-weight:600";
...
async function ppcRunBuilder(){
  ...
  try{
    ...
    if(v.errors&&v.errors.length){
      html+='<div style="margin-top:8px;font-size:12px;color:var(--red)"><b>Errors (must fix before upload):</b><br>'+v.errors.map(esc).join("<br>")+'</div>';
    }
    if(v.warnings&&v.warnings.length){
      html+='<div style="margin-top:8px;font-size:12px;color:var(--warn)"><b>Warnings:</b><br>'+v.warnings.map(esc).join("<br>")+'</div>';
    }
...
async function ppcRunHarvest(){
  ...
  try{
    ...
    html+='Ready for harvest: <b style="color:var(--ok)">'+(t.harvest_ready||0)+'</b> new converting terms (excluded '+j.excluded_already_targeted+' already-targeted)<br>';
    html+='Ready for negation: <b style="color:var(--red)">'+(t.negatives_ready||0)+'</b> past-$10 zero-order terms';
    html+='</div>';
    html+='<div style="margin:10px 0;display:flex;flex-wrap:wrap;gap:6px;font-size:12px">';
    Object.keys(c).forEach(k=>{
      // Shared .tone-* classes rather than inline hex with alpha glued on -- see
      // the tone-chip block in dashboard.css for why that pattern had to go.
      const tone = k==='CONVERTING'?'tone-ok'
                 : (k==='OVER-$10-CUT'||k==='CLICKS-NO-SALE')?'tone-bad'
                 : (k==='CONVERTS-BUT-HIGH-ACOS'||k==='HIGH-SPEND-WATCH')?'tone-warn'
                 : 'tone-info';
      html+='<span class="tonechip '+tone+'">'+esc(k)+': '+c[k]+'</span>';
    });
    html+='</div>';
...
async function ppcRunDeliv(){
  ...
  try{
    ...
    (j.file_summary||[]).forEach(f=>{
      const tag = f.family ? '<span style="color:var(--ok)">'+esc(f.family)+'</span>' : '<span style="color:var(--warn)">unknown</span>';
    ...
    // Analysis text
    if(j.reply){
      html+='<div style="font-weight:600;margin:12px 0 6px;color:var(--ok)">Executive note</div>';

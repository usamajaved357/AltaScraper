...
function LOGIC_REGISTRY(){
  ...
  return {
    ...
    cogs: { title:"How COGS & the profit estimate work", steps:[
      `<b>Three answers, in this order.</b> Asked what one unit cost you, the app tries each in turn and stops at the first that answers — <code>domain/cogs.resolve</code>:
       <ol style="margin:6px 0 2px 18px">
         <li><b>A cost you set</b> — typed into the COGS cell, or uploaded on the cost sheet. Always wins, because you typed it <i>because</i> the SKU was wrong or absent, and a number parsed out of a name must never overrule a person.</li>
         <li><b>The cost in the SKU's own name</b> — the <code>8.00</code> in <code>8.00_3Days_B0G1K5B7QS</code>. The generator writes what the item cost at its source into the SKU when it builds it, so most items already carry their cost and need nothing done.</li>
         <li><b>Not known</b> — a hand-made SKU like <code>46 pcs wrench</code> carries no number.</li>
       </ol>`,
      `<b>0.00 in a SKU means UNKNOWN, not free.</b> The generator writes <code>0.00</code> when it had no cost to write. Treated as a real cost of nothing, that item looks infinitely profitable — which is precisely the one somebody would then order more of. So it is read as "no cost known", and every figure built on it says so instead of quietly assuming.`,

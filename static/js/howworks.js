...
function LOGIC_REGISTRY(){
  ...
  return {
    ...
    cogs: { title:"How COGS & the profit estimate work", steps:[
      `<b>Two answers, in this order.</b> Asked what one unit cost you, the app tries each in turn and stops at the first that answers — <code>domain/order_cogs.resolve</code>:
       <ol style="margin:6px 0 2px 18px">
         <li><b>A cost you set against that one order</b> — typed on the Orders page, or uploaded on the per-order cost sheet. Wins outright, and applies to that order alone.</li>
         <li><b>A cost you set against the product</b> — typed into the COGS cell, or uploaded on the cost sheet. Applies to every order of that product, past and future.</li>
         <li><b>Not known</b> — and not known is not zero. The profit is left blank.</li>
       </ol>`,
      `<b>Nothing is read out of a SKU name any more.</b> The app used to fall back to the <code>8.00</code> in <code>8.00_3Days_B0G1K5B7QS</code>, and to the supplier price the repricer happened to have recorded. Both are gone, by instruction: <i>"lets remove the cogs from sku things entirely, lets keep it simple."</i> A number parsed out of a name was a guess dressed as data — right only for SKUs this app generated itself, impossible for one typed by hand like <code>46 pcs wrench</code>, and with nothing on any screen to tell the two apart. Every value that parse was supplying was written into the cost store first, so no figure went blank when it went.`,
      `<b>An old cost still shows its age.</b> Costs frozen onto orders before that change are labelled "an old cost, read from the SKU name" rather than left to look like something the app still does. Replace one by typing over it.`,
      `<b>Not known is not zero, anywhere.</b> A SKU with no cost is left out of profit rather than counted as pure profit. That is why the screens say how much of the catalogue they could cost — a profit figure covering four fifths of your SKUs is useful, the same figure presented as if it covered all of them is not.`,
      ...
      `<b>The arithmetic, and where each one is used.</b> There are two, and they answer different questions:
       <table style="margin:6px 0;border-collapse:collapse;font-size:11.5px">
         <tr><td style="padding:3px 10px 3px 0;vertical-align:top"><b>On this screen</b><br><span class="cc">an estimate, before you sell</span></td>
             <td style="padding:3px 0"><code>profit = price − cost − 15% of price</code><br>
             <span class="cc">The 15% is a stand-in for Amazon's referral fee. Category rates differ and there are other charges, so this is a margin guide for deciding a price — never a record of what you earned.</span></td></tr>
         <tr><td style="padding:3px 10px 3px 0;vertical-align:top"><b>On Sales and Orders</b><br><span class="cc">measured, after you sell</span></td>
             <td style="padding:3px 0"><code>profit = what Amazon paid you − the fees Amazon charged − cost</code><br>
             <span class="cc">Every figure there comes off Amazon's own settlement, so the fee is the real one for that order rather than 15%. Nothing is estimated and nothing is assumed.</span></td></tr>
       </table>
       Both use the same cost, from the same three-step lookup above. Where they disagree it is the FEE that differs, not the cost.`,
      `<b>Where a cost you set is kept.</b> In <code>cogs_overrides.json</code> beside your config, keyed by account and SKU — so the same SKU can cost different amounts on two accounts, and one account's costs can be cleared without touching another's. Nothing is written to Amazon; a cost is yours and stays here.`,
      `<b>Do they actually reach everything?</b> Yes, and it is worth saying because they once did not: Sales and Orders used to read a second, permanently empty copy of the overrides, so a cost typed here changed the listings screen and not the profit figures. One store now serves every screen (<code>domain/cogs_store.py</code>). The <i>How costs work</i> button shows this account's real breakdown, which is the way to check it rather than take this sentence's word for it.` ]},
    // ---------- PRICING RULE ----------
    pricing_rule: { title:"How your selling price is worked out", steps:[
      `<b>The rule, in full.</b> <code>price = what the item cost you + Amazon's referral fee + enough on top to return at least 20% on the cash you put in</code>. The floor is whichever of those requirements is highest, and it can only ever raise a price.`,
      `<b>The £3 postage and £2 ads allowances are GONE, on purpose.</b> They used to be added here AND subtracted from reported profit, which made the app contradict itself: the same nestwell order read <b>+£2.58, ROI 29%</b> in the orders list and <b>−£2.32, ROI −26%</b> when opened, the second having charged it £5 of allowances no money ever left the account for. Both were "right"; neither was true. <i>"i want to be shown the profit as the truth"</i>. They remain as per-SKU parameters, at zero, because you really may post a parcel — when that number is known it belongs in the seller's own per-unit charges, where it is subtracted from measured profit rather than invented on everyone's behalf.`,
      `<b>What replaced the flat £1: a percentage.</b> With those at zero, "cost + Amazon's fee" is break-even, and a repricer that prices to break-even loses money on every sale. So the requirement is <b>20% ROI</b> — a share of the cash you put in, which means the same thing on a £12 item and a £60 one, where £1 meant 8% and under 2%. Visible in the repricer as <code>target_roi_pct</code>, changeable and switchable per SKU.`,
      ...
      `<b>Change the 20%</b> per SKU in the repricer, or its default in <code>listing/pricing.py</code>. Changing the default re-prices everything, so it is a visible edit rather than a settings box.` ]},
    // ---------- SUBMIT LIVE ----------

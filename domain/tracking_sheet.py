...
It will not give them back. Measured against nestwell_goods, not assumed:
getOrder carries OrderStatus and the ship-by dates and no tracking; getOrderItems
carries QuantityShipped and no tracking; the All Orders report has 33 columns and
none of them is tracking or carrier; MerchantFulfillment can read back a label
THIS app bought but has no "list my shipments" call, so it cannot discover one.
...
def apply_sheet(config_path, workspace_id, marketplace, headers, rows):
    ...
    i_carrier = _sheets.pick(headers, CARRIER_COLS)
    ...
    out = {"ok": True, "set": 0, "blank": 0, "unknown_order": 0,
           ...
           "columns": {"order": (headers[i_order] if i_order >= 0 else ""),
                       ...
                       "carrier": (headers[i_carrier] if i_carrier >= 0 else "")}}
    if i_order < 0:
        out["ok"] = False
    ...
    for r in rows or []:
        oid = cell(r, i_order)
        tn = cell(r, i_track)
        carrier = cell(r, i_carrier) if i_carrier >= 0 else ""

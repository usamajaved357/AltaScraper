...
def register(app, *, CONFIG_PATH, _state):
    ...
    def input_upload_template():
        ...
        w.writerow([
            ...
            "the app has read the real price from eBay. PUT YOUR BARCODE IN "
            "upc — that column also answers to ean, gtin, barcode or ean_upc "
            "as a heading, so name it whichever you use. It is sent to Amazon "
            "as the product identifier exactly as given; the app NEVER invents "
            "a barcode, and a row with no barcode and no GTIN exemption is "
            "refused by Amazon, which is the correct outcome — the exemption "
            "is only ever claimed when you tick it yourself. Leave "
            "selling_price "
